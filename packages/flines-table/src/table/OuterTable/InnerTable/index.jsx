...
import { cns, getCellId } from '../Core';

export default class InnerTable extends PureComponent {
    constructor(props) {
        super(props);
    ...
    renderColHeaderRow() {
        ...
        return (
            <Row key={rowId} className={`${cns}rowForColHeaders`}>
                <Column key={tableHeaderKey} className={`${cns}colForTableHeader`}>
                    {this.renderTableHeader()}
                </Column>
                {colIds &&
                    colIds.map((colId, colIndex) => (
                        <Column key={colId}
                            className={`${cns}colForColHeader`}
                            colId={colId}
    ...
    renderColResizerRow() {
        ...
        return (
            <Row key={rowId} className={`${cns}rowForColResizers`}>
                <Column key={emptyHeaderKey} className={`${cns}colForColResizerEmpty`} />
                {colIds &&
                    colIds.map((colId, colIndex) => (
                        <Column key={colId}
                            colId={colId}
    ...
    renderCellRow(rowId, rowIndex) {
        ...
        return (
            <Row key={rowId}
                isSelected={selectedRowIndex === rowIndex && selectedColIndex === -1}>
                <Column key={rowHeaderColId} className={`${cns}colForRowHeader`}>
    ...
    render() {
        ...
        return (
            <table className={`${cns}InnerTable`} style={customStyle}>
                <thead>
